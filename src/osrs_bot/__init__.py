...
def _find_workspace() -> Path:
    """Find the workspace directory (where .kiro/ lives)."""
    # 1. Env var
    ws = os.environ.get("OSRS_BOT_WORKSPACE")
    if ws:
    ...
    for parent in [p] + list(p.parents):
        if (parent / ".kiro").is_dir():
            return parent
    # 3. Default
    return Path.home() / "github/osrs-ai"
...
_raw_conf = _load_conf_file(WORKSPACE / ".osrs-bot.conf")

DREAMBOT_PROJECT = Path(os.environ.get(
    "OSRS_BOT_DREAMBOT_REPO",
    _raw_conf.get("dreambot_repo", str(Path.home() / "github/osrs-dreambot")),
)).expanduser().resolve()

SCRIPTS_SRC = DREAMBOT_PROJECT / "src/main/java/scripts"
SPECS = WORKSPACE / ".kiro/specs/scripts"
TRACKER = WORKSPACE / ".kiro/scripts.json"
TEMPLATE = SPECS / "TEMPLATE.md"
LOCAL_LOGS = WORKSPACE / ".kiro/logs"
...
def load_config() -> dict:
    ...
    for k in conf:
        if k in _raw_conf:
            conf[k] = Path(_raw_conf[k]).expanduser()
...
def run_cmd(cmd: list[str], cwd: Path | None = None, check: bool = True) -> subprocess.CompletedProcess:
    return subprocess.run(cmd, cwd=cwd or WORKSPACE, check=check)
...
def gradle(args: list[str]):
    ...
    if platform.system() == "Windows" and wrapper_jar.exists():
        ...
        cmd = [
            ...
            "--project-dir", str(DREAMBOT_PROJECT.resolve()),
        ] + args
    else:
...
""")
    ...
    print(f"  source:  {pkg_dir.relative_to(DREAMBOT_PROJECT)}")
...
def _changed_scripts(tracker: dict) -> list[str]:
    ...
    result = subprocess.run(
        ...
        cwd=DREAMBOT_PROJECT, capture_output=True, text=True, check=False,
    )
    ...
    for sid, entry in tracker.get("scripts", {}).items():
        ...
        if any(f.startswith(f"src/main/java/{pkg}/") for f in changed_files):
            changed.append(sid)
    return changed
...
def _git_commit_push(cwd: Path, message: str, paths: list[str] | None = None):
    """Stage, commit, and push in a repo. Returns True if anything was pushed."""
    if paths:
        subprocess.run(["git", "add"] + paths, cwd=cwd, check=False)
    else:
        subprocess.run(["git", "add", "-A"], cwd=cwd, check=False)
    result = subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=cwd, check=False)
...
def cmd_deploy(args):
    ...
    if args.quick:
        # Push dreambot source changes
        _git_commit_push(DREAMBOT_PROJECT, f"quick deploy: {ts}", ["src/"])
        # Push workspace tracker/specs/live
        _git_commit_push(WORKSPACE, f"quick deploy: {ts}",
                         [".kiro/live", ".kiro/scripts.json", ".kiro/specs"])
        print(f"\n✓ Quick deploy complete — {ts}")
        print(f"  Windows: osrs-bot run")
        return

    # Full deploy — push both repos
    if _git_commit_push(DREAMBOT_PROJECT, f"deploy: {ts}"):
        print("✓ Pushed dreambot")
    if _git_commit_push(WORKSPACE, f"deploy: {ts}"):
        print("✓ Pushed workspace")
...
def cmd_run(args):
    if platform.system() == "Windows":
        ...
        # Pull latest code
        run_cmd(["git", "pull"], cwd=DREAMBOT_PROJECT, check=False)
        run_cmd(["git", "pull"], cwd=WORKSPACE, check=False)
...
def copy_images(sid: str, image_paths: list[str]) -> list[str]:
    ...
    for p in image_paths:
        ...
        saved.append(str(dest.relative_to(WORKSPACE)))
    return saved
...
def cmd_lint(args):
    ...
    for f in SCRIPTS_SRC.rglob("*Script.java"):
        ...
        if "@ScriptManifest" not in text:
            issues.append(f"  ✗ {f.relative_to(DREAMBOT_PROJECT)}: missing @ScriptManifest")
        if "extends TaskScript" in text and "addNodes" not in text:
            issues.append(f"  ✗ {f.relative_to(DREAMBOT_PROJECT)}: TaskScript without addNodes() in onStart()")
    if issues:
...
def cmd_logs_pull(args):
    ...
    print(f"✓ Copied log to {dest.relative_to(WORKSPACE)} (local only, gitignored)")
...
def cmd_push(args):
    ...
    result = subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=WORKSPACE, check=False)
    if result.returncode != 0:
        run_cmd(["git", "commit", "-m", "sync: bugs, notes, specs"], cwd=WORKSPACE)
        run_cmd(["git", "push"], cwd=WORKSPACE)
